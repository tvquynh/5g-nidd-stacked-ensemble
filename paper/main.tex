...
\title{Lightweight Stacked Ensemble for AI-Enabled 5G Network Intrusion Detection:\\
A Latency--Accuracy Pareto Analysis}

\author{
\IEEEauthorblockN{Van-Quynh Trinh\IEEEauthorrefmark{1},
Trong-Thua Huynh\IEEEauthorrefmark{1}\thanks{Corresponding author: huynhtrongthua@ptit.edu.vn.},
De-Thu Huynh\IEEEauthorrefmark{2},
and Ngoc-Hieu Le\IEEEauthorrefmark{1}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}Posts and Telecommunications Institute of Technology (PTIT), Vietnam\\
\IEEEauthorrefmark{2}Saigon International University (SIU), Vietnam}
}
...
spanning $\{1, 16, 64, 256, 1024, 4096\}$. The stacked ensemble matches
or improves the best single base learner on macro-F1 while remaining
practical for CPU-only deployment in the user-plane function (UPF) or
multi-access edge computing (MEC) tier. We make the artefact reviewer-runnable
and release a deployment recommendation table that maps operating points
(latency budget, accuracy floor) to the configuration that meets them.
\end{abstract}
...
macro-F1 0.99, with the stacked ensemble matching or improving the best
single base learner. Under the cross-station split BS1$\rightarrow$BS2
the gap between methods widens: the stacked ensemble narrows the
generalisation drop more than any single base learner, supporting the
intuition that combining complementary inductive biases helps when the
...
plots throughput against batch size across two decades. Per-sample
latency is dominated by the tree learners at small batch sizes (where
LightGBM's $C$-implementation has the lowest constant overhead) and by
the MLP's dense matrix multiplications at larger batches. The stacked
ensemble costs the sum of its base learners plus a negligible $O(K^2)$
softmax over the meta features; the resulting frontier point sits to the
right of the bases on the latency axis but above them on the accuracy
axis whenever the gap is meaningful, allowing operators to choose by
constraint.
...
\begin{itemize}
    \item \emph{Latency-critical} (sub-microsecond per flow, batch~1):
        LightGBM alone offers the lowest constant-overhead inference
        because the histogram-binned trees evaluate as a sequence of
        cache-friendly integer comparisons. The accuracy floor is the
        macro-F1 reported in Table~\ref{tab:main} for LightGBM.
    \item \emph{Balanced edge} (a few microseconds per flow, batch
        256--1024): the stacked ensemble dominates: its absolute
        latency overhead vs.\ the fastest base is comparable to the
        intra-batch variance of a single base, while macro-F1 is at
        least as high as the best single base on the random split and
        higher under cross-station shift.
    \item \emph{Accuracy-first analytics} (large batches, $\geq 4096$):
        the stacked ensemble is the recommended configuration; the
        throughput advantage of any single base is amortised over the
        batch, so the marginal cost of stacking is small relative to
        the per-class F1 gains.
\end{itemize}
